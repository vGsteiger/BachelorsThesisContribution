...
/**
 * Implementation of the log normal temporal scoring algorithm as described in the thesis.
 */
class LogNormalScoringAlgorithm : ScoringAlgorithm() {
    override val algorithmId: String = "LNA"
            ...
            if (scoredSegmentsLists[objectId]!!.containsKey(innerContainerId)) {
                ...
                listFromElement.forEach {
                    if (tmpSegmentStorage.containsKey(it.segmentId)) {
                        ...
                        val tmpScore = calculateLogNormalScore(
                            currentEndTime,
                            it,
    ...
    private fun calculateLogNormalScore(
        ...
        val m = timeDifference.toFloat() + 0.6f
        val s = 0.5f
        var score = ((1f / ((-x + m) * s * sqrt(2f * Math.PI))) * exp(-(ln(-x + m).pow(2f)) / (2f * s)))
